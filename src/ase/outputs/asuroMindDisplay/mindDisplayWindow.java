 ...
 * @author Tobi
 */
public class mindDisplayWindow extends JPanel implements IOutputEventCreator,IEventdrivenOutput, java.awt.event.KeyListener, ase.IThreadControl, java.lang.Runnable {

    private int intDistance;
    private int offsetX;
    private int offsetY;
    private String instanzName ;
    private boolean bolFollowOtherVisualOutputs;
    private Hashtable<String,IEventdrivenOutput> myEventOutputs;
    private JFrame thisFrame;
    private Point worldSize;
    private Thread thisThread;
    private boolean bolKeepAlive;
    private int intTakeEveryXSynchronizedSnapshots;
    private int intSynchronizedSnapshotCounter;
    private String strAsuroInstanceName;
    private int intSleepTime;
    private AsuroMindDisplay myParent;

    public mindDisplayWindow(Point sizeOfWorld,String name, String strAsuroInstanceName, int threadSleepTime, AsuroMindDisplay parent) {
        intDistance = 30;
        offsetX = 0;
        offsetY = 0;
        ...
        this.strAsuroInstanceName =  strAsuroInstanceName;
    ...
    protected void paintComponent(java.awt.Graphics g) {
        ...
        if(myParent != null){
            offsetX = (int)(myParent.getCurrentPosition(strAsuroInstanceName).x - getWidth() / 2);
            offsetY = (int)(myParent.getCurrentPosition(strAsuroInstanceName).y - getHeight() / 2);
            myParent.drawObjects(g, 1.0 / intDistance, offsetX, offsetY, strAsuroInstanceName);
        }
    }
    ...
    public void run() {
        ...
        {
            try {
                thisThread.sleep(intSleepTime);
                repaint();
            } catch (InterruptedException ex) {
                notifyMonitors(messageType.ERROR, new Object[] {"Thread von " + strAsuroInstanceName + " hatte einen Unfall! Er wird jetzt weiter ausgeführt.",ex});
            }
        }
    }
...
public void registerOutput(IEventdrivenOutput newOutput)
    ...
    protected void takeSnapshot(String strFolder, int thumbWidth, int thumbHeight, int snapshotID)
    {
     ...
     public void takeSynchronizedSnapshot(int intSnypshotID, String strFolderPath, int intThumbWidth, int intThumbHeight) {
        ...
        {
            this.notifyMonitors(messageType.DEBUG, new Object[] {getInstanceName() + " is taking synchronized snapshot",intSnypshotID,strFolderPath});
